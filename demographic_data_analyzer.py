...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = (num_min_workers['salary'].value_counts(normalize=True)['>50K'] * 100).round(1)

    # What country has the highest percentage of people that earn >50K?
    country_earnings = df[df['salary'] == '>50K']['native-country'].value_counts()
    country_total = df['native-country'].value_counts()

    country_percentage = (country_earnings / country_total * 100).fillna(0).round(1)

    highest_earning_country = country_percentage.idxmax()
    highest_earning_country_percentage = country_percentage.max().round(1)
